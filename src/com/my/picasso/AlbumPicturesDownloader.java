...
public class AlbumPicturesDownloader extends AsyncTask<Void, Void, ArrayList<PicasaEntry>> {                          
    ...
    public AlbumPicturesDownloader(PicasaListAdapter adapter,String url,String auth) {           
    ...
    protected ArrayList<PicasaEntry> doInBackground(Void... params) {
		 ...
		 HttpGet getAction = new HttpGet(albumUrl);
         getAction.setHeader("Authorization", "GoogleLogin auth="+googleKey);
         Log.d("LoadAlbumPictures","Started");
         try{
	         ...
	         if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK ){
                 try {
	    	         ...
	    	         while ((responseLine = responseReader.readLine())!=null){
	    	        	 ...
	    	        	 Log.d("PicasaAlbum","collect resp");
	    	        }
	...
	private ArrayList<PicasaEntry> parsePictures(String xml)
    {
      ...
      try {
        ...
        for (int i = 0; i < entries.getLength(); i++) {
            ...
            //node "content"
            temp = element.getChildNodes().item(6);

        	PicasaEntry p = new PicasaEntry(getCharacterDataFromElement((Element)element.getElementsByTagName("title").item(0)),
        			                      getCharacterDataFromElement((Element)element.getElementsByTagName("summary").item(0)),
